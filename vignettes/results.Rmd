...
For each of these 103 events, the data were analysed in the same way.  The methods, and highlighted results, are given in in this document.  Extra details for each of the 103 analyses are reported in [Appendix 2](app2_index.html). 
...
4. _Nitrogenous_ bisphosphonates and non-nitrogenous bisphosphonates (LIST) considered as different treatments. 
...
For the selected model, a corresponding classical meta-analysis is also performed based on the direct comparisons alone.  (TODO might be nicer to use Bayesian methods for this, for consistency with the model checking procedure?) 
...
Table 2 gives odds ratios from direct and selected network meta-analysis models for all events, sorted by clinical category, including non-significant results.  Effects significant under either model, as listed in Table 1, are shown here in red. 
...
* Put serious AE counts in ortab 

# TODO 

* Absolute treatment effects - e.g. risk differences.  For this, we need to define a baseline group. 

	- Women with breast cancer who might be recommended bisphosphonates 
	- ie similar to those included in studies of bisphosphonates 
	- though reported rates in control groups are highly variable 
	- take baseline rates from selected higher quality studies with good reporting, eg AZURE? 
	- paper:  two baselines, one for healthy women, one for cancer. 
	- select baseline from control pops of highest quality studies. top bin. 
	- single number for low risk and high risk baseline 

* Haven't considered potential study biases 
- sensitivity analysis including data high quality

Right so for both of these need to identify high quality studies. 

table(bpaeraw$reporting == "Complete") # 23 out of 58 of them 

Right so shall we do this SA, and at the same time extract estimates of baseline rates for obs only. Posterior median. 
Or just a simple MA? just cos wwe need a reasonable single number not a dist. 
Yes.  Studies that has obs-only controls 

what about healthy women - from horizon and fit? But they don't have all events. 

* Connect to AZURE individual-level analysis, reported in another document (tidy up)

	- qualitatively, similar kinds of events are important


* Upload healthy women analysis

* Me to check funny numbers against double checked spreadsheet 
...
2.  forest plots for table 1 and 2 data
