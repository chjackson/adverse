...
mares <- mares %>% filter(event %in% sigevents)
orsh <- orsh %>% filter(event %in% pull(mares, event))
mares <- mares %>% filter(event %in% pull(orsh, event))
orsh <- orsh %>% full_join(mares)

## FIXME why no ONJ, cancer
